...
export class ContactMenuCompanyComponent implements OnInit {
  ...
  listTbData = {
    ...
    listButton: [
      {
        id: BUTTON_TYPE.ADD_LIST_MAIL,
        name: "Thêm vào ds mail",
        color: "primary",
      },
    ...
    this.mService
      ...
      .then((data) => {
        if (data[ParamsKey.STATUS] == STATUS.SUCCESS) {
          ...
          let listParams = [];
          if (this.stageID != "")
            listParams.push({ key: "stepID", value: this.stageID });
          if (this.cityID != "")
            listParams.push({ key: "cityID", value: this.cityID });
          if (this.timeFrom != "")
            listParams.push({ key: "timeFrom", value: this.timeFrom });
          if (this.timeTo != "")
            listParams.push({ key: "timeTo", value: this.timeTo });
          if (this.userIDFind != "")
    ...
    if (event.btnType == BUTTON_TYPE.ASSIGN) {
      ...
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          this.mService
            .getApiService()
            .sendRequestASSIGN_COMPANY_OWNER(res, event.data)
            .then((data) => {
      ...
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          this.mService
            .getApiService()
            .sendRequestADD_MAIL_LIST_DETAIL(res, event.data)
            .then((data) => {
              if (data.status == STATUS.SUCCESS) {
                this.mService.publishEvent(EVENT_PUSH.SELECTION, true);
  ...
  onClickImport() {
    UploadFileModule.getInstance().openFileImport((res) => {
      this.mService
        .getApiService()
        .sendRequestIMPORT_DATA(JSON.stringify(res))
        .then((data) => {
          if (data.status == STATUS.SUCCESS) {
            this.onLoadData(
              1,
  ...
  onClickCell(event) {
    if (event) {
      if (event.clickDetail == CLICK_DETAIL.CONTACT) {
        this.mService.publishPageRoute("contact-detail", {
        ...
        this.mService.publishPageRoute("company-detail", {
