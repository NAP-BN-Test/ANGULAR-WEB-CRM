...
export class EmailCampainDetailComponent implements OnInit {
  ...
  onLoadData() {
    ...
    this.mService
      .getApiService()
      .sendRequestGET_MAIL_CAMPAIN_DETAIL(this.campainID)
      .then((data) => {
    ...
    this.mService
      ...
      .then((data) => {
        if (data.status == STATUS.SUCCESS) this.listMailList = data.array;
      });
  ...
  handleSendMailBtn() {
    ...
    let timeSendBefore = localStorage.getItem(
      LOCAL_STORAGE_KEY.SEND_EMAIL + this.campainID
    )
      ? Number(
          localStorage.getItem(LOCAL_STORAGE_KEY.SEND_EMAIL + this.campainID)
        )
      : new Date().getTime() - 1800000;

    let timeSpan = now - timeSendBefore;

    this.disableBtnSendMail = timeSpan >= 60000;
    ...
    if (!this.disableBtnSendMail)
      setTimeout(() => {
        ...
        let intervalTimeRemain = setInterval(() => {
          ...
          let minute =
            Math.floor(timeSpanSecond / 60) > 9
              ? Math.floor(timeSpanSecond / 60)
              : "0" + Math.floor(timeSpanSecond / 60);
          let second =
            timeSpanSecond % 60 > 9
              ? timeSpanSecond % 60
              : "0" + (timeSpanSecond % 60);
  ...
  onClickSave() {
    let obj = {
      ...
      endTime: moment(
        this.timeEnd.year + "-" + this.timeEnd.month + "-" + this.timeEnd.day
      ).format("YYYY-MM-DD"),
      body: this.mObj.body,
  ...
  onClickSend() {
    const dialogRef = this.dialog.open(ConfirmSendEmailComponent, {
      data: {
        campainName: this.mObj.name,
        mailListName: this.mObj.mailListName,
      },
    });
  ...
  onClickSendTest() {
    ...
    this.mService
      .getApiService()
      .sendRequestADD_MAIL_SEND(obj, true)
      .then((data) => {
        if (data.status == STATUS.SUCCESS) {
          this.mService.showSnackBar(data.message);
        }
  ...
  onClickDelete() {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res) {
        ...
        this.mService
          .getApiService()
          .sendRequestDELETE_MAIL_CAMPAIN(JSON.stringify(listID))
          .then((data) => {
            if (data.status == STATUS.SUCCESS) {
              this.location.back();
            }
  ...
  checkEmailVerify(email: string) {
    this.mService
      .getApiService()
      .sendRequestCHECK_VERIFY_EMAIL(email)
      .then((data) => {
        if (data.status == STATUS.SUCCESS) this.btnVerify = true;
        else this.btnVerify = false;
      });
  }
