...
export class CompanyInfoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.mService
      .getApiService()
      .sendRequestGET_DETAIL_COMPANY(this.mID + "")
      .then((data) => {
  ...
  onInputChange(event, type) {
    let value;
    if (type != 6 && type != 8) value = event.target.value;
  ...
  onClickFollow() {
    this.mService
      .getApiService()
      .sendRequestFOLLOW_COMPANY(this.mObj.id, !this.mObj.follow ? true : null)
      .then((data) => {
  ...
  onClickDelete() {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res) {
        ...
        this.mService
          .getApiService()
          .sendRequestDELETE_COMPANY(JSON.stringify(listID))
          .then((data) => {
            if (data.status == STATUS.SUCCESS) {
              this.location.back();
            }
